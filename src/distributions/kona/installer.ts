...
export class KonaDistribution extends JavaBase {
  ...
  private async getAvailableReleases(): Promise<IKonaRelease[]> {
    ...
    const availableReleases =
      this.chooseReleases(this.getOs(), this.getArch(), releaseInfo);
  ...
  private async fetchReleaseInfo(): Promise<IKonaReleaseInfo | null> {
    ...
    try {
      ...
      return (
        await this.http.getJson<IKonaReleaseInfo>(releasesInfoUrl)
      ).result;
    } catch (err) {
      core.debug(
        `Fetching Kona release info from the URL: ${releasesInfoUrl} failed with the error: ${(err as Error).message}`
      );
      return null;
    }
  }
